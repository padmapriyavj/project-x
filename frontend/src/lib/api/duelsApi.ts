...
export async function createDuel(token: string, body: DuelCreateBody): Promise<DuelCreateResponse> {
  return apiFetchJson<DuelCreateResponse>('/api/v1/duels', {
    method: 'POST',
...
export async function joinDuel(token: string, roomId: string): Promise<DuelJoinResponse> {
  return apiFetchJson<DuelJoinResponse>(`/api/v1/duels/${encodeURIComponent(roomId)}/join`, {
    method: 'POST',
...
export async function createDuelAttempt(token: string, roomId: string): Promise<DuelAttemptResponse> {
  return apiFetchJson<DuelAttemptResponse>(`/api/v1/duels/${encodeURIComponent(roomId)}/attempts`, {
    method: 'POST',
    headers: authHeaders(token),
  })
}
...
export async function markDuelActive(roomId: string): Promise<void> {
  const res = await apiFetch(`/api/v1/duels/${encodeURIComponent(roomId)}/mark-active`, {
    method: 'POST',
  })
  if (!res.ok) {
...
export async function settleDuel(
  ...
  return apiFetchJson<DuelSettleResponse>(`/api/v1/duels/${encodeURIComponent(roomId)}/settle`, {
    method: 'POST',
    headers: { ...authHeaders(token), 'Content-Type': 'application/json' },
    body: JSON.stringify(body),
  })
